...
The admin-managed allowlist that gates real access. Document ID is the account's email address as Firebase Auth stores it — for an office account (the primary sign-in method today, see "Staff accounts" below) this is a **synthetic email** derived from the office id (`{officeId}@office.malaikahoney.local`, e.g. `kampala@office.malaikahoney.local`), generated by `officeIdToEmail()`; for a phone+password account (hidden) it's `{digits}@staff.malaikahoney.local` via `phoneToEmail()`; for a Google account (hidden) it's the real Google account email (e.g. `jokello@gmail.com`) — all three in `public/js/lib/auth.js`.
...
| Field | Type | Notes |
|---|---|---|
| `email` | string | Redundant copy of the doc ID (synthetic office/phone-account email or real Google email — see `allowedStaff` above) |
| `displayName` | string | The office id (offices have no separate display name field), the name staff typed when creating a phone account, or the Google account's display name |
...
Approving a request (`/admin/approvals`, `adminApprovals.js` `approveRequest`) creates the corresponding `allowedStaff/{email}` document (if it doesn't already exist — see `firestore.rules` "allowedStaff" for why a plain create-or-skip is needed, not a blind write) and sets `status: 'approved'`; rejecting only sets `status: 'rejected'` — no `allowedStaff` document is created, so the person stays blocked. Records are kept (not deleted) for an audit trail, and the fetch behind `/admin/approvals` filters to `status == 'pending'` client-side.

## Staff accounts (office + code, primary; phone+password and Google Sign-In, both hidden)

**One shared code per field office** is the primary sign-in method (`public/js/lib/auth.js` `signInWithOfficeCode`, `public/js/screens/login.js`) — staff pick their office from a dropdown (populated from `fieldOffices`, below) and type that office's code, deliberately minimal for staff who don't read/speak English well. Unlike every other sign-in method this app has had, office accounts are **not self-service** — an admin creates the account directly (Firebase Auth email/password, with a synthetic email derived from the office id: `officeIdToEmail()`, `{officeId}@office.malaikahoney.local`) and sets the code as its password, per [[Config-Management]] "Field office provisioning". The office's code effectively *is* the Firebase Auth password; there's no separate "code" field stored anywhere.

Phone+password (`signInWithPhone`/`createAccountWithPhone`) and Google Sign-In (`signInWithGoogle`) are both **not deleted** — fully functional in the codebase but hidden from the login screen behind `PHONE_SIGNIN_ENABLED`/`GOOGLE_SIGNIN_ENABLED` in `public/js/lib/constants.js` (both currently `false`). Flipping either flag back to `true` restores that method's UI (stacked below the office form) with no other changes needed.

Whichever method is used, signing in only creates a Firebase Auth session — it does **not** by itself grant access to farmer/purchase data. An account only gets real access once an admin approves it, adding its email (real or synthetic) as a document in the `allowedStaff` collection (see above). Until then, the app shows a short "Approval Needed" screen with the signed-in identity displayed — an office id, a phone number, or a real address for Google (`identityLabel()` strips the synthetic domain back down to just the office id/phone number so the raw internal address is never shown to staff). The in-app `/admin/approvals` screen (see `signupRequests` above) works identically regardless of which method created the request; for offices specifically, the recommended provisioning flow adds the `allowedStaff` document *at the same time* as the Auth account, so staff never see this screen in the normal case (see [[Config-Management]]) — the request/approval flow only kicks in as a safety net if that step is skipped.

**Trade-off, accepted:** a shared office code is a weaker identity signal than a per-person account — everyone at that office uses the same login, so `registeredBy`/`recordedBy` on farmer/purchase documents now reflects the *office*, not which individual staff member actually entered a record. The client chose this deliberately for simplicity; see [[Risk-Register]] for the accountability and code-rotation trade-offs this implies.

Signing out (`signOutStaff()`) ends the Firebase Auth session outright, so a different account can sign in on the same shared device afterward.

Sessions persist locally (`browserLocalPersistence`) so a device that has signed in at least once while online can keep using the app fully offline afterward — `waitForAuthReady()` resolves from the cached session before the app decides whether to show the login screen. The *authorization* result (whether they're on the allowlist) is similarly cached locally (`localStorage`, keyed by uid) the first time it's confirmed online, so a returning approved account is never blocked from an authenticated screen just because it's offline — see `public/js/lib/auth.js` `refreshAuthorization`/`isAuthorizedLocally`. If an admin later removes an office from the allowlist, that only takes effect for a given device the next time it manages to reach the server (an inherent, accepted lag for an offline-first app — see [[Risk-Register]]).

### `fieldOffices/{officeId}`

Populates the office dropdown on the Login screen — doc ID is a slug (e.g. `kampala`) used to derive the account's synthetic email (see above). Same shape and admin-editing pattern as the other reference-data collections below (`{ label, order, active }`, hardcoded single-placeholder fallback in `referenceData.js` until real offices are added), with one important difference:

**This is the only collection in the whole schema readable with no sign-in at all** (`allow read: if true;` in `firestore.rules`) — it has to be, since the picker renders *before* anyone is authenticated. Only ever put office display names/ordering here, nothing more sensitive — see [[Risk-Register]].

Unlike the other reference-data collections, an admin can also `create` (not `update`/`delete`) a new `fieldOffices` document directly from within the app — see `/admin/add-office` (`public/js/screens/addOffice.js`) and [[Config-Management]] "Field office provisioning". This writes all three pieces a new office needs in one step: the `fieldOffices` document, the matching Firebase Auth account (via `auth.js` `createOfficeAccount`), and the `allowedStaff` document.
...
`allowedStaff` allows a signed-in user to `get` their own document (to check their own status), and an **admin** to `get` any document and `create` new entries (approving a `signupRequests` entry from within the app) — `list`/`delete` remain fully client-blocked, and `update` is allowed only against a user's own document and only for the `fcmTokens` field (push-notification opt-in, see [[Push-Notifications]]); revoking access or changing someone's role stays a Console-only action. `adminApprovals.js`'s `approveRequest` doesn't actually rely on that admin-`get` allowance to check whether a document already exists before creating one — it just attempts the `create` directly and treats a `permission-denied` result as "already exists, nothing to grant" rather than a real error, which stays correct regardless of what the `get` rule allows. `signupRequests` allows any signed-in user to `create` their own entry and read (`get`) it back, but only an admin can `list` the pending queue or `update` (resolve) a request.

`devices/{deviceCode}` additionally allows `create` only if the document doesn't already exist yet, and disallows `update`/`delete` entirely, so a device code can never be silently overwritten. The retired `counters/{counterId}` collection still allows `read` for approved staff (harmless, unused) but no longer allows `write`. `fieldOffices/{officeId}` is the sole exception to "every read requires approved staff" — `allow read: if true`, deliberately, so the Login screen's office picker works with no sign-in (see "Staff accounts" above and [[Risk-Register]]) — it additionally allows `create` (not `update`/`delete`) for an admin, powering the in-app `/admin/add-office` screen.
