...
| 2.16 | In-app UI for managing reference data collections (2.14) and the New Farmer form schema | `fieldOffices` now has one — admins can **Add Office** from within the app (v0.6.5, see [[Config-Management]] "Field office provisioning") — but adding/editing `products`, `grades`, `paymentMethods`, `farmSizes`, `districts`, `newFarmerFields`, and editing/deactivating existing offices, are all still manual Firestore Console steps; natural fit for the admin app (Milestone 3) |
