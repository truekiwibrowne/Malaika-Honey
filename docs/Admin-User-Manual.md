...
2. **Sign in with your office and its code** (see "Logging in" below). The first time you sign in, you'll see a short tutorial — you can skip it any time.
...
1. Pick your office from the list.
2. Type your office's code.
3. Tap **Sign In**.
...
5. To sign out, go to Home and tap **Sign Out** at the bottom. If you have unsynced work, the app will warn you before signing out, since a queued purchase or registration may not reach the office if you sign out before it finishes syncing — wait for the badge to show **Synced** first if you can. Signing out fully ends your session, so a different office can then sign in on the same shared device.

Your office's account is set up for you ahead of time — you don't need to create one yourself. If you ever see "Approval Needed" after signing in, tell your admin (see [[Config-Management]] "Field office provisioning") and try **Check Again**.

## Approving new sign-ins (admins only)

If your account has been made an admin, you'll see an **Approve Requests** button on Home — a number in brackets shows how many sign-ins are waiting for a decision. This normally shouldn't happen for offices set up correctly (see [[Config-Management]]), but it's there as a safety net.

1. Tap **Approve Requests**.
2. Each pending request shows the office, and when it first tried to sign in.
3. Tap **Approve** to let it into the app, or **Reject** to turn it away (it can still try signing in again later — rejecting isn't permanent).
...
**Do I need to log in?**
Yes — pick your office and type its code (see "Logging in" above). You only need internet the first time; after that, you can keep working offline on that phone.

**I signed in but I'm stuck on "Approval Needed" — what do I do?**
This shouldn't normally happen for an office set up correctly. Tell your admin, then tap **Check Again**.

**I forgot the office code — what do I do?**
Ask your admin — they can look it up or set a new one for you in Firebase Console's Authentication tab.
