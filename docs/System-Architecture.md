...
- Firestore transactions (used for the FRN counter and lifetime-stat updates) require a live round trip to the backend and reject when there's no connection — they do **not** work offline. `public/js/lib/db.js` therefore tries the transactional path first (safe under concurrent online use — see [[Database-Schema]] "counters/frnCounter") and, whenever that fails (offline, or any other error), falls back to a plain batched write built from the locally cached counter/farmer data. That batch's commit is deliberately not awaited to completion — Firestore only resolves a write's promise once the server acknowledges it, so awaiting it would hang the UI until connectivity returns — and the queued write syncs automatically once the device is back online. (See [[Risk-Register]] R3 for the resulting rare multi-device-offline-simultaneously edge case.)
