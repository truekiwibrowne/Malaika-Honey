...
- **Static HTML/CSS/JS, no build step, no framework runtime dependency.** The brief calls for "simple." A framework (React/Vue) adds a build pipeline and bundle weight that buys nothing for 3 screens and increases the barrier to a second developer picking this up later. The app is structured as a single-page shell (`index.html`) with plain ES modules per screen, so it stays simple to read but doesn't sprawl into unmaintainable spaghetti — see `public/js/screens/`. `app.js` dynamically `import()`s each screen's module inside its own route handler rather than importing all of them up front — native ES modules already support this with zero bundler, and it's what keeps first-load fast: only the current route's screen loads before first paint, not all twelve (this is about the shell's own JS, separate from Firestore's data cache described under "Offline behavior in detail" below).
...
- **Firebase Authentication (one shared code per field office, primary; phone+password and Google Sign-In, both hidden), not a custom login system.** Office+code is the primary method today, chosen for simplicity given staff literacy constraints — it's still Firebase Auth's built-in email/password provider under the hood (via a synthetic email derived from the office id), so no session management, password resets, or custom login backend needed to be built from scratch. Unlike the two hidden methods, office accounts are admin-provisioned, not self-service (see [[Config-Management]] "Field office provisioning"). Phone+password and Google Sign-In both remain fully implemented but hidden behind flags (`PHONE_SIGNIN_ENABLED`/`GOOGLE_SIGNIN_ENABLED`), restorable without a rewrite. Whichever method is used, real data access is additionally gated by an admin-managed `allowedStaff` allowlist checked server-side in `firestore.rules` — sign-in and authorization are deliberately two separate steps (see [[Database-Schema]] "Staff accounts"). The SDK's local session persistence is what makes "sign in once online, then work offline indefinitely" possible, and the authorization result is cached the same way so a returning approved account is never blocked offline.
...
Not every CDN dependency is precached this way, only ones the field-critical, offline path actually needs. `sw.js` splits these into `FIREBASE_SDK_URLS` (required before any screen can render, always precached) and `OTHER_CDN_URLS` (feature-specific, but still precached because that feature must keep working offline — currently just `jsqr`, used by the in-app QR scanner, `public/js/lib/qrScanner.js`, so a staff member can still scan a farmer's card while offline after at least one prior online load). Dependencies used only for lower-frequency, less offline-critical actions — `jspdf` and `qrcode`, both used only when actually generating/printing a Farmer Card PDF — are deliberately **not** precached; see [[Risk-Register]] for the accepted tradeoff.
